                  ...
                  length.seg, jackknife = FALSE, trace = FALSE, ...)
{
  ...
  for (n.seg in 1:length(segments)) {
    ...
    ## Fit the model:
    fit <- rosa.fit(Xtrain, X.concat, Y[-seg,,drop=FALSE], Y.add[-seg,,drop=FALSE], ncomp, common.comp, weights[-seg], fixed.order, NULL, NULL, canonical)
    ...
    if(response.type == "categorical"){
      ...
      cvClass[seg,] <- classifyRosa(fit, Y.cat[-seg,,drop=FALSE], Xt, ncomp, 'lda')
    }
  }
  if (trace) cat("\n")
